...
def mpv():
    if paste() != "":
        if os.system("mpv " + paste()):
            os.system("notify-send 'Play.py' 'Is this link from youtube?'")
...
elif len(sys.argv) > 1 and sys.argv[1] in ('-m', '--mpv'):
    mpv()
else:
    print('Usage: python -m pyperclip [-c | --copy] | [-p | --paste] | [-m | --mpv]')
    print()
